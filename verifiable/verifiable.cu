...
void prepareInput1(
  ...
  case ncclFp8E4M3: if (rccl_float8_useFnuz) { CASE_TY(__hip_fp8_e4m3_fnuz);}
  else { CASE_TY(__hip_fp8_e4m3);}
  case ncclFp8E5M2: if (rccl_float8_useFnuz) { CASE_TY(__hip_fp8_e5m2_fnuz);}
...
void ncclVerifiableVerify(
  ...
  *bad_elt_n = 0;
  ...
  #endif
  #if HAVE_ncclfp8_HOST
  case ncclFp8E4M3: if (rccl_float8_useFnuz) { CASE_TY(__hip_fp8_e4m3_fnuz, uint8_t);}
  else { CASE_TY(__hip_fp8_e4m3, uint8_t);}
  case ncclFp8E5M2: if (rccl_float8_useFnuz) { CASE_TY(__hip_fp8_e5m2_fnuz, uint8_t);}
